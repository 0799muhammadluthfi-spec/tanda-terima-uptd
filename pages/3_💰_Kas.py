    ...
    # ══════════════════════════════════════════
        ...
        if sumber_anggaran == "UANG KAS (DI TANGAN)":
            ...
            if nominal > last_kas and nominal > 0:
                st.warning(
                    f"⚠️ Nominal keluar ({rupiah(nominal)}) "
                    f"melebihi sisa kas di tangan ({rupiah(last_kas)})"
                )
            ...
            if nominal > last_atm and nominal > 0:
                st.warning(
                    f"⚠️ Nominal keluar ({rupiah(nominal)}) "
                    f"melebihi sisa di ATM ({rupiah(last_atm)})"
                )
            ...
            if nominal > last_penyedia and nominal > 0:
                st.warning(
                    f"⚠️ Nominal keluar ({rupiah(nominal)}) "
                    f"melebihi sisa di Penyedia ({rupiah(last_penyedia)})"
                )
