...
def hitung_ringkasan_kas(df_kas):
    try:
        ...
        d["_jenis"] = d["Jenis_Transaksi"].astype(str).str.strip().str.upper()
        d["_nominal"] = d["Nominal"].apply(to_float)

        total_masuk = d[d["_jenis"] == "MASUK"]["_nominal"].sum()
        total_keluar = d[d["_jenis"] == "KELUAR"]["_nominal"].sum()
