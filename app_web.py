...
<style>
    ...
    .main .stTextInput label, .main .stTextInput input,
    .main .stNumberInput label, .main .stNumberInput input,
    .main .stSelectbox label,
    .main .stCheckbox label, .main .stCheckbox label span,
    .main .stRadio label,
    ...
    .main h1 {
        ...
        color: #0f172a !important; letter-spacing: -0.03em !important; line-height: 1.25 !important;
    }
    .main h2 {
    ...
    .main h3 {
        font-size: 0.98rem !important; font-weight: 600 !important; color: #334155 !important;
    }
    ...
    .main .stCheckbox label span {
        font-size: 0.84rem !important; font-weight: 500 !important; color: #374151 !important;
    }
    ...
    .main .stTextInput input::placeholder, .main .stNumberInput input::placeholder {
    ...
    [data-testid="stMetricLabel"] {
        ...
        text-transform: uppercase !important; letter-spacing: 0.03em !important; color: #6b7280 !important;
    }
    ...
    .main .stDownloadButton > button {
        ...
        padding: 8px 16px !important; line-height: 1.4 !important;
        white-space: nowrap !important; overflow: visible !important; min-height: 40px !important;
    ...
    .streamlit-expanderHeader span:not([data-testid="stExpanderToggleIcon"]):not([data-testid="stExpanderToggleIcon"] *) {
        font-family: 'Inter', sans-serif !important;
        font-size: 0.86rem !important; font-weight: 600 !important; color: #1e293b !important;
    }
    ...
    [data-testid="stDataFrame"] th {
        ...
        text-transform: uppercase !important; color: #4b5563 !important; background: #f9fafb !important;
    }
    ...
    /* ============================================================
       ANIMASI KONTEN - STAGGERED FADE IN
       ============================================================ */
    .anim-item {
        opacity: 0;
    ...
    /* Stagger otomatis untuk elemen-elemen utama */
    ...
    .main > div > div > div > div > div:nth-child(9)  { animation: itemFadeUp 0.45s 0.36s cubic-bezier(0.22,1,0.36,1) both; }
    .main > div > div > div > div > div:nth-child(10) { animation: itemFadeUp 0.45s 0.40s cubic-bezier(0.22,1,0.36,1) both; }
    .main > div > div > div > div > div:nth-child(n+11){ animation: itemFadeUp 0.45s 0.44s cubic-bezier(0.22,1,0.36,1) both; }
    ...
    [data-testid="column"]:nth-child(2) [data-testid="stMetric"] { animation-delay: 0.12s; }
    [data-testid="column"]:nth-child(3) [data-testid="stMetric"] { animation-delay: 0.18s; }
...
# ==========================================
# 3. FUNGSI PDF — TIDAK DIUBAH SAMA SEKALI
# ==========================================
...
def cetak_tanda_terima_parkir(data) -> BytesIO:
    ...
    # Konversi MPP ke int dengan aman
    def safe_int(val):
        try:
            s = str(val).strip()
...
def halaman_parkir(menu):
    ...
    # Inisialisasi default agar tidak UnboundLocalError
    dt_user  = None
    baris    = pd.DataFrame()
    idx      = None
    nama_p   = "-"
    ...
    if not baris.empty:
        ...
        if not df_petugas_sama.empty:
            ...
            sisa_r2 = pd.to_numeric(
                df_petugas_sama.loc[idx_terakhir].get("Sisa_Stok_R2", 0), errors='coerce')
            sisa_r4 = pd.to_numeric(
                df_petugas_sama.loc[idx_terakhir].get("Sisa_Stok_R4", 0), errors='coerce')
    ...
    if menu == "INPUT REKAP":
        ...
        if st.session_state.get("show_confirm_parkir"):
            st.warning(
                f"⚠️ Data setoran **{tgl_input_user}** sudah pernah diisi! Yakin ingin menimpanya?"
            )
            col_c1, col_c2 = st.columns(2)
                ...
                for i, row in df_terfilter.iterrows():
                    ...
                    with st.expander(
                        ...
                        with ck:
                            ...
                            st.markdown(
                                f"**KHUSUS** {badge_k}<br>"
                                f"R2: {row['Khusus_Roda_R2']} &nbsp;|&nbsp; R4: {row['Khusus_Roda_R4']}",
                                unsafe_allow_html=True
                            )
                            if stat_k != "SUDAH":
                        ...
                        with cm_col:
                            ...
                            st.markdown(
                                f"**MPP** {badge_m}<br>"
                                f"R2: {row['MPP_Roda_R2']} &nbsp;|&nbsp; R4: {row['MPP_Roda_R4']}",
                                unsafe_allow_html=True
                            )
                            if stat_m != "SUDAH":
...
def halaman_welcome():
    ...
    <style>
        ...
        @keyframes logoPulse {{
            0%, 100% {{ transform: scale(1);    filter: drop-shadow(0 8px 24px rgba(0,0,0,0.12)); }}
            50%       {{ transform: scale(1.04); filter: drop-shadow(0 12px 32px rgba(59,130,246,0.18)); }}
        }}
        .welcome-title {{
            animation: textSlideUp 0.7s cubic-bezier(0.22, 1, 0.36, 1) 0.35s forwards;
        ...
        .welcome-subtitle {{
            animation: textSlideUp 0.7s cubic-bezier(0.22, 1, 0.36, 1) 0.55s forwards;
            opacity: 0; will-change: transform, opacity;
            font-family: 'Inter', sans-serif; font-size: 1.05rem; font-weight: 500;
            color: #64748b; margin: 8px 0 0 0;
        }}
        .welcome-line {{
            animation: lineExpand 0.65s cubic-bezier(0.22, 1, 0.36, 1) 0.75s forwards;
            opacity: 0; width: 0; height: 3px;
            background: linear-gradient(90deg, #3b82f6, #60a5fa, #93c5fd);
            margin: 20px auto; border-radius: 2px; will-change: width, opacity;
        }}
        ...
        .welcome-hint {{
            animation: textSlideUp 0.7s cubic-bezier(0.22, 1, 0.36, 1) 0.95s forwards;
            opacity: 0; will-change: transform, opacity;
            font-family: 'Inter', sans-serif; font-size: 0.88rem;
            font-weight: 400; color: #94a3b8; margin: 8px 0 0 0;
        }}
        .welcome-credit {{
            animation: creditFloat 0.7s cubic-bezier(0.22, 1, 0.36, 1) 1.2s forwards;
            opacity: 0; will-change: transform, opacity;
