...
# Link Logo HSS yang lebih stabil (Direct Link)
...
# (Fungsi get_next_no, buat_pdf_full, cetak_overprint tetap sama...)
...
def halaman_pengantaran():
    ...
    cols_berkas = st.columns(3); sel_berkas = []
    ...
    with st.form("form_pengantaran", clear_on_submit=False):
        ...
        if st.form_submit_button("💾 SIMPAN DATA", type="primary"):
            if not no_urut.strip() or not nama_toko or not nama_pemilik: st.error("❌ Data Bintang (*) Wajib Diisi!")
            else:
                ...
                if safe_update("DATA_SK", df_baru):
                    st.session_state["last_data"] = new_row; st.session_state["last_berkas"] = sel_berkas; st.success("✅ Berhasil Disimpan!"); st.rerun()
    if "last_data" in st.session_state:
        st.divider(); last = st.session_state["last_data"]
    ...
    st.divider(); st.subheader("📊 DATA TABEL GOOGLE SHEETS")
...
def halaman_pengambilan():
    ...
    if no_cari:
        ...
        if not hasil.empty:
            data_row = hasil.iloc[0]; sudah_ambil = data_row["Tanggal_Pengambilan"] not in ["-", "nan", "NAN", ""]
            if sudah_ambil:
    ...
    st.divider(); st.subheader(f"📊 DAFTAR BERKAS BELUM DIAMBIL ({len(df_belum_diambil)} Berkas)")
    st.dataframe(df_belum_diambil.sort_values(by="No", ascending=True), use_container_width=True, hide_index=True)

# ==========================================
# 6. MODUL PARKIR (PERBAIKAN ERROR DUPLICATE KEY)
# ==========================================
def halaman_parkir(menu_aktif):
    ...
    if menu_aktif == "INPUT REKAP":
        ...
        try:
            if len(tgl_bersih.split("-")[-1]) == 2: tgl_obj = datetime.strptime(tgl_bersih, "%d-%m-%y")
            else: tgl_obj = datetime.strptime(tgl_bersih, "%d-%m-%Y")
            tgl_std = tgl_obj.strftime("%d-%m-%Y"); tgl_alt = f"{tgl_obj.day}-{tgl_obj.month}-{tgl_obj.year}"
        except: tgl_std = tgl_bersih; tgl_alt = tgl_bersih
        baris_cocok = df_parkir[(df_parkir["Tanggal"].astype(str) == tgl_std) | (df_parkir["Tanggal"].astype(str) == tgl_alt)]
        if not baris_cocok.empty:
            idx = baris_cocok.index[0]; nama_ptgs = baris_cocok.iloc[0]["Nama_Petugas"]
            ...
            with st.form("form_parkir"):
                c1, c2 = st.columns(2)
                with c1: pk_r2 = st.number_input("AMBIL BARU R2", 0); tot_r2 = st.number_input("TERJUAL R2", 0); mpp_r2 = st.number_input("MPP R2", 0)
                with c2: pk_r4 = st.number_input("AMBIL BARU R4", 0); tot_r4 = st.number_input("TERJUAL R4", 0); mpp_r4 = st.number_input("MPP R4", 0)
                if st.form_submit_button("💾 UPDATE DATA", type="primary"):
                    sisa_n_r2 = (pk_r2 + sisa_r2_lama) - tot_r2; sisa_n_r4 = (pk_r4 + sisa_r4_lama) - tot_r4
        ...
        if df_pending.empty: st.info("Semua rekap sudah dikonfirmasi.")
            ...
            # Gunakan 'index' dari dataframe sebagai ID Unik tombol
                ...
                with st.expander(f"📦 Nomor: {row['No']} - {row['Nama_Petugas']} ({row['Tanggal']})"):
                    ...
                    with ck:
                        ...
                        if row["Status_Khusus"] == "BELUM":
                            # KUNCI PERBAIKAN: key menggunakan 'index' dataframe agar tidak duplikat
    ...
    st.divider(); st.subheader("📊 LOG REKAP PARKIR")
    if not df_parkir.empty: st.dataframe(df_parkir.sort_values(by="No", ascending=False), use_container_width=True, hide_index=True)

# ==========================================
# 7. MAIN APP (SPLASH SCREEN & LOGO HSS)
# ==========================================
def main():
    ...
    if not st.session_state["sudah_masuk"]:
        ...
        with col2:
            # Gunakan URL alternatif jika wikipedia diblokir
        ...
        with st.sidebar:
            ...
            else: menu = st.radio("MENU PARKIR:", ["INPUT REKAP", "KONFIRMASI"])
